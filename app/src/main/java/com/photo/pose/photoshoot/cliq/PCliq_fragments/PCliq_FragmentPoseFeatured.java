...
public class PCliq_FragmentPoseFeatured extends Fragment {
    ...
    private void getWallpaperData() {
        if (methods.isNetworkAvailable()) {
            ...
            Call<PCliq_ItemPoseList> call = PCliq_APIClient.getClient().create(PCliq_APIInterface.class).getWallpapersByFeatured(methods.getAPIRequest(PCliq_Constant.URL_WALLPAPER_BY_FEATURED, 1, color_ids, wallType, "", "", "", "", "", "", "", "", new PCliq_SharedPref(getActivity()).getUserId(), ""));
...
//                                    int abc = arrayList.lastIndexOf(null);
//                                    if (((arrayList.size() - (abc + 1)) % PCliq_Constant.nativeAdShow == 0) && (response.body().getArrayListWallpaper().size() - 1 != i || response.body().getArrayListWallpaper().size() != response.body().getTotalRecords())) {
//                                        arrayList.add(null);
//                                    }
//                                }
                            }
